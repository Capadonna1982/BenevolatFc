...
function seedDB(){
  ...
  return {
    settings:{ hoursGoal:15, creditMode:'approval', withdrawHours:48, logo:null },
...
function renderCoachEvents(c){
  c.innerHTML=`
    <div class="page-head"><h2>${t('navEvents')}</h2><p>${t('appTagline')}</p></div>
...
function addNeedDraft(){
  ...
  const hours = hv===''?null:parseFloat(hv);
...
function renderMembers(c){
  ...
  c.innerHTML=`
    ...
    <div class="card">
      ...
      <table><thead><tr><th>${t('name')}</th><th>${t('email')}</th><th>${t('category')}</th><th>${t('hoursShort')}</th></tr></thead><tbody>
      ${players.length?players.map(u=>`<tr><td><div style="display:flex;align-items:center;gap:10px">${avatarHTML(u)}<b>${esc(u.first)} ${esc(u.last)}</b></div></td>
        ...
        <td>${catSelect(u)}</td>
        <td><span class="chip" style="background:var(--brand-light);color:var(--brand-dark)">${playerHours(u.id)} / ${DB.settings.hoursGoal} ${t('hoursShort')}</span></td></tr>`).join(''):`<tr><td colspan="4" style="color:var(--muted);text-align:center;padding:16px">—</td></tr>`}
      </tbody></table>
    </div>
    ...
    ${invited.length?`<div class="card">
      ...
      ${invited.map(u=>`<tr><td><div style="display:flex;align-items:center;gap:10px">${avatarHTML(u)}<b>${esc(u.first)} ${esc(u.last||'')}</b></div></td>
        ...
        <td><button class="btn btn-ghost btn-sm" onclick="copyInvite('${u.id}')">🔗 ${t('copyLink')}</button></td></tr>`).join('')}
      </tbody></table>
    </div>`:''}`;
}
